    ...
    if not os.path.exists(os.path.join(par_path, "ml-1m", "ratings.dat")):
        ...
        while not answered:
            ...
            if choice in ["yes", "y"]:
                ...
                answered = True
                print("refused to download, then exit...")
                sys.exit(1)
    else:
        print("Dataset already downloaded.")


def _extract_and_preprocess_data(par_path=None):
    ...
    if not os.path.exists(os.path.join(par_path, "ml-1m", "merged_data.csv")):
...
def prepare_data(par_path=None, feat=False, data="ml-1m"):
    ...
    _download(par_path)
    _extract_and_preprocess_data(par_path)
